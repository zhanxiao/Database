...
BEGIN TRY
	...
	BEGIN
		IF EXISTS (SELECT 1 FROM Op_Forecast WHERE StartStationID = @StationID AND OperateTime >= @yestoday AND OperateTime < @today AND LineCode = @LineCode)
		BEGIN
			--下属网点
			...
			FROM Op_Forecast 
			WHERE StartStationID = @WarehouseStationId AND OperateTime >= @yestoday AND OperateTime < @today AND LineCode = @LineCode
			...
			BEGIN
				SELECT @TotalSettlementWeight = ISNULL(SUM(TotalJSWeight),0), @TotalVolume = ISNULL(SUM(TotalBulk),0), @TotalActualWeight = ISNULL(SUM(TotalWeight),0),
					@TotalBillNum = ISNULL(SUM(TotalBillNum),0), @TotalGoodsNum = ISNULL(SUM(TotalGoodsNum),0)
				FROM Op_Forecast a
				JOIN Bas_Line b on a.StartStationID = b.StartStationID and a.NextStationID = b.EndStationID and a.LineCode = b.LineCode
				WHERE b.LineID = @detailsLineID AND a.StartStationID = @WarehouseStationId AND OperateTime >= @yestoday AND OperateTime < @today
